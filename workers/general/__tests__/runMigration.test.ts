...
describe("runMigration worker", () => {
  ...
  it("throws error if migration not found", async () => {
    ...
    await expect(runMigration(job)).rejects.toThrow(
      "Migration not found: nonexistent-migration",
    );
  });
  ...
  it("creates and updates migration run records", async () => {
    ...
    const runs = await MigrationRunService.find({
      match: { migrationId: "test-migration-123" },
    });
