...
export default {
  ...
  description:
    "Sets createdBy field for all prompts that lack it. Assigns the first team admin as the creator.",
  ...
  async up(db: Db): Promise<MigrationResult> {
    ...
    console.log(
      `Found ${promptsWithoutCreatedBy.length} prompts without createdBy`,
    );
    ...
    for (const prompt of promptsWithoutCreatedBy) {
      ...
      console.log(
        `  Prompt.team type: ${typeof prompt.team}, value: ${prompt.team}`,
      );
      ...
      try {
        ...
        if (!team) {
          console.warn(
            `  ❌ Team not found for prompt ${prompt._id} (looking for team._id = ${prompt.team}) - DELETING ORPHAN`,
          );
        ...
        console.log(
          `  Searching for team admin with teams.team = ${prompt.team}`,
        );
        ...
        if (creator) {
          console.log(
            `  ✓ Found team admin: ${creator._id} (${creator.username})`,
          );
        } else {
        ...
        if (!creator) {
          ...
          if (creator) {
            console.log(
              `  ✓ Found super admin: ${creator._id} (${creator.username})`,
            );
          } else {
            console.warn(
              `  ❌ No admin or super admin found for prompt ${prompt._id}`,
            );
            failed++;
            continue;
          }
        ...
        console.log(
          `  ✓ Updated prompt with createdBy: ${creator._id} (matched: ${updateResult.matchedCount}, modified: ${updateResult.modifiedCount})`,
        );
        migrated++;
      } catch (error) {
