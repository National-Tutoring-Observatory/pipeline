...
export default {
  ...
  description:
    "Backfill performedBy and performedByUsername from context.target for audit records",
  ...
  async up(db: Db): Promise<MigrationResult> {
    ...
    const auditsMissingPerformedBy = await auditCollection
      .find({
        ...
        "context.target": { $exists: true },
      })
      .toArray();

    console.log(
      `Found ${auditsMissingPerformedBy.length} audits missing performedBy`,
    );
    ...
    for (const audit of auditsMissingPerformedBy) {
      try {
        ...
        console.log(
          `  ✓ Backfilling audit ${audit._id}: performedBy=${userId}, performedByUsername=${username}`,
        );
