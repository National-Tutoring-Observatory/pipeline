...
export default {
  ...
  description:
    "Builds RunSnapshot objects for all runs that have completed setup but lack snapshots. This ensures backward compatibility with runs created before the snapshot feature.",
  ...
  async up(db: Db): Promise<MigrationResult> {
    ...
    for (const run of runsWithoutSnapshots) {
      try {
        ...
        if (!promptVersion) {
          console.warn(
            `⚠ Prompt version not found for run ${run._id}: prompt=${run.prompt}, version=${run.promptVersion}`,
          );
          failed++;
          continue;
        }
        ...
        if (!prompt) {
          console.warn(
            `⚠ Prompt not found for run ${run._id}: prompt=${run.prompt}`,
          );
          failed++;
          continue;
        }
