...
describe('collectionMerge.route loader', () => {
  ...
  it('redirects to / when project not found', async () => {
    ...
    const res = await loader({
      ...
      params: { projectId: new Types.ObjectId().toString(), collectionId: 'any' }
    } as any);
...
describe('collectionMerge.route action', () => {
  it('returns 403 when user cannot manage project', async () => {
    ...
    const req = new Request('http://localhost/', {
      ...
      body: JSON.stringify({ intent: 'MERGE_COLLECTIONS', payload: { sourceCollectionIds: ['any'] } })
    });
  ...
  it('merges collections and redirects to collection detail', async () => {
    ...
    const req = new Request('http://localhost/', {
      ...
      body: JSON.stringify({ intent: 'MERGE_COLLECTIONS', payload: { sourceCollectionIds: [sourceCollection._id] } })
    });
    ...
    expect((resp as Response).headers.get('Location')).toBe(`/projects/${project._id}/collections/${targetCollection._id}`);
  ...
  it('merges multiple collections', async () => {
    ...
    const req = new Request('http://localhost/', {
      method: 'POST',
      headers: { cookie: cookieHeader, 'content-type': 'application/json' },
      body: JSON.stringify({ intent: 'MERGE_COLLECTIONS', payload: { sourceCollectionIds: [source1._id, source2._id] } })
    });
