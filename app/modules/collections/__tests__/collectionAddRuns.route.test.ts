...
describe('collectionAddRuns.route action', () => {
  it('returns 403 when user cannot manage project', async () => {
    ...
    const req = new Request('http://localhost/', {
      ...
      body: JSON.stringify({ intent: 'ADD_RUNS', payload: { runIds: ['any'] } })
    });
  ...
  it('adds runs and redirects to collection detail', async () => {
    ...
    const req = new Request('http://localhost/', {
      ...
      body: JSON.stringify({ intent: 'ADD_RUNS', payload: { runIds: [run._id] } })
    });
    ...
    expect((resp as Response).headers.get('Location')).toBe(`/projects/${project._id}/collections/${collection._id}`);
