...
describe("collectionsList.route loader", () => {
  ...
  it("redirects to / when project not found", async () => {
    ...
    const res = await loader({
      request: new Request(
        "http://localhost/projects/" + fakeProjectId + "/collections",
  ...
  it("returns collections for authorized users", async () => {
    ...
    const res = await loader({
      request: new Request(
        "http://localhost/projects/" + project._id + "/collections",
        { headers: { cookie: cookieHeader } },
      ),
  ...
  it("creates collection successfully", async () => {
    ...
    const req = new Request(
      "http://localhost/projects/" + project._id + "/collections",
      {
        ...
        body: JSON.stringify({
          intent: "CREATE_COLLECTION",
          payload: { name: "Test Collection", annotationType: "PER_UTTERANCE" },
        }),
      },
    );
