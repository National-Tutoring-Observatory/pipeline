...
describe('SystemAdminAuthorization', () => {
  ...
  describe('FeatureFlags.canManage', () => {
    it('allows super admins to manage feature flags', () => {
      expect(SystemAdminAuthorization.FeatureFlags.canManage(superAdminUser)).toBe(true);
    });

    it('denies regular users from managing feature flags', () => {
      expect(SystemAdminAuthorization.FeatureFlags.canManage(regularUser)).toBe(false);
    });
  ...
  describe('Queues.canManage', () => {
    it('allows super admins to manage queues', () => {
      expect(SystemAdminAuthorization.Queues.canManage(superAdminUser)).toBe(true);
    });

    it('denies regular users from managing queues', () => {
      expect(SystemAdminAuthorization.Queues.canManage(regularUser)).toBe(false);
    });
