...
describe("splitMultipleSessionsIntoFiles", () => {
  it("throws error if no files provided", async () => {
    await expect(splitMultipleSessionsIntoFiles({ files: [] })).rejects.toThrow(
      "No files provided",
    );
  });
...
{"session_id":"session_002","role":"Tutor","content":"Welcome","sequence_id":1}`;
    ...
    const jsonlFile = new File([jsonlContent], "test.jsonl", {
...
session_001,Tutor,Hello,1`;
    ...
    const result = await splitMultipleSessionsIntoFiles({
      files: [csvFile, jsonlFile],
    });
...
session_001,Tutor,Hello,1`;
  ...
  it("rejects unsupported file extensions", async () => {
    ...
    await expect(
      splitMultipleSessionsIntoFiles({ files: [txtFile] }),
    ).rejects.toThrow('Unsupported file type: "test.txt"');
  });
