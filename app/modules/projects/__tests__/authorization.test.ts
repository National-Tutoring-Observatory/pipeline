  ...
  describe('Runs', () => {
    describe('canManage', () => {
      it('allows team members to manage runs in their team', () => {
        expect(ProjectAuthorization.Runs.canManage(teamAdminUser, 'team-1')).toBe(true);
        expect(ProjectAuthorization.Runs.canManage(teamMemberUser, 'team-1')).toBe(true);
      });

      it('denies super admins to manage runs in a team', () => {
        expect(ProjectAuthorization.Runs.canManage(superAdminUser, 'team-1')).toBe(false);
      });

      it('denies non-members from managing runs', () => {
        expect(ProjectAuthorization.Runs.canManage(nonTeamUser, 'team-1')).toBe(false);
        expect(ProjectAuthorization.Runs.canManage(teamAdminUser, 'team-999')).toBe(false);
      });

      it('denies null users from managing runs', () => {
        expect(ProjectAuthorization.Runs.canManage(null, 'team-1')).toBe(false);
      });
    });
  });
  ...
  describe('Annotations', () => {
    describe('canManage', () => {
      it('allows team members to manage annotations in their team', () => {
        expect(ProjectAuthorization.Annotations.canManage(teamAdminUser, 'team-1')).toBe(true);
        expect(ProjectAuthorization.Annotations.canManage(teamMemberUser, 'team-1')).toBe(true);
      });

      it('denies super admins to manage annotations in a team', () => {
        expect(ProjectAuthorization.Annotations.canManage(superAdminUser, 'team-1')).toBe(false);
      });

      it('denies non-members from managing annotations', () => {
        expect(ProjectAuthorization.Annotations.canManage(nonTeamUser, 'team-1')).toBe(false);
        expect(ProjectAuthorization.Annotations.canManage(teamAdminUser, 'team-999')).toBe(false);
      });

      it('denies null users from managing annotations', () => {
        expect(ProjectAuthorization.Annotations.canManage(null, 'team-1')).toBe(false);
      });
    });
  });
