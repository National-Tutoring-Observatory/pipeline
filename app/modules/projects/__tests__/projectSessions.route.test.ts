...
describe("projectSessions.route loader", () => {
  ...
  it("redirects to / when project not found", async () => {
    ...
    const res = await loader({
      request: new Request("http://localhost/projects/" + fakeProjectId + "/sessions", { headers: { cookie: cookieHeader } }),
  ...
  it("returns sessions for authorized user", async () => {
    ...
    const res = await loader({
      request: new Request("http://localhost/projects/" + project._id + "/sessions", { headers: { cookie: cookieHeader } }),
