...
describe("projectFiles.route loader", () => {
  ...
  it("redirects to / when project not found", async () => {
    ...
    const res = await loader({
      request: new Request("http://localhost/projects/" + fakeProjectId + "/files", { headers: { cookie: cookieHeader } }),
  ...
  it("returns files for authorized user", async () => {
    ...
    const res = await loader({
      request: new Request("http://localhost/projects/" + project._id + "/files", { headers: { cookie: cookieHeader } }),
