...
describe("projectCreateRun.route loader", () => {
  ...
  it("redirects to / when project not found", async () => {
    ...
    const res = await loader({
      request: new Request("http://localhost/projects/" + fakeProjectId + "/create-run", { headers: { cookie: cookieHeader } }),
  ...
  it("redirects to / when user is not in project team", async () => {
    ...
    const res = await loader({
      request: new Request("http://localhost/projects/" + project._id + "/create-run", { headers: { cookie: cookieHeader } }),
