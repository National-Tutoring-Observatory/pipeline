...
export async function action({ request }: Route.ActionArgs) {
  ...
  switch (intent) {
    case 'ASSIGN_SUPER_ADMIN': {
      ...
      if (!UserManagementAuthorization.canAssignSuperAdminToUser({ target: targetUser, performer: user })) {
        return data(
    ...
    case 'REVOKE_SUPER_ADMIN': {
      ...
      if (!UserManagementAuthorization.canRevokeSuperAdminFromUser({ target: targetUser, performer: user })) {
        return data(
          { errors: { general: 'Cannot perform this action' } },
          { status: 400 }
        );
...
export default function UserManagementRoute({ loaderData }: Route.ComponentProps) {
  ...
  useEffect(() => {
    if (fetcher.state === 'idle' && fetcher.data) {
      if (fetcher.data.success && fetcher.data.intent === 'ASSIGN_SUPER_ADMIN') {
        toast.success('User promoted to super admin');
        addDialog(null);
      } else if (fetcher.data.success && fetcher.data.intent === 'REVOKE_SUPER_ADMIN') {
  ...
  const onAssignSuperAdminClicked = (targetUserId: string) => (reason: string) => {
    fetcher.submit(
      JSON.stringify({
  ...
  const onRevokeSuperAdminClicked = (targetUserId: string) => (reason: string) => {
    fetcher.submit(
      JSON.stringify({
