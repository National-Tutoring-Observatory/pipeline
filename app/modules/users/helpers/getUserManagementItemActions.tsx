...
export default (item: User, currentUser: User): CollectionItemAction[] => {
  ...
  if (UserManagementAuthorization.canAssignSuperAdminToUser({ target: item, performer: currentUser })) {
    actions.push({
  ...
  if (UserManagementAuthorization.canRevokeSuperAdminFromUser({ target: item, performer: currentUser })) {
    actions.push({
