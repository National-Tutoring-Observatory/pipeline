...
export class AuditService {
  ...
  static async create(data: Omit<AuditRecord, '_id' | 'createdAt' | 'performedByUsername'> & { performedByUsername?: string }): Promise<AuditRecord> {
    if (!data.performedBy) {
  ...
  static async createSystem(data: Omit<AuditRecord, '_id' | 'createdAt' | 'performedBy' | 'performedByUsername'>): Promise<AuditRecord> {
    const doc = await AuditModel.create({
      ...data,
