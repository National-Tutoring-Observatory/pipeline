...
describe("TeamAuthorization", () => {
  ...
  describe("canUpdate", () => {
    ...
    it("denies super admins to update any team", () => {
      ...
      expect(TeamAuthorization.canUpdate(superAdminUser, "team-999")).toBe(
        false,
      );
    });
    ...
    it("denies non-members from updating a team", () => {
      ...
      expect(TeamAuthorization.canUpdate(teamAdminUser, "team-999")).toBe(
        false,
      );
    });
  ...
  describe("canDelete", () => {
    ...
    it("denies super admins to delete any team", () => {
      ...
      expect(TeamAuthorization.canDelete(superAdminUser, "team-999")).toBe(
        false,
      );
    });

    it("denies team admins from deleting other teams", () => {
      expect(TeamAuthorization.canDelete(teamAdminUser, "team-999")).toBe(
        false,
      );
    });
  ...
  describe("Users", () => {
    describe("canView", () => {
      it("allows super admins to view team users", () => {
        expect(TeamAuthorization.Users.canView(superAdminUser, "team-1")).toBe(
          true,
        );
      });

      it("allows team admins to view team users", () => {
        expect(TeamAuthorization.Users.canView(teamAdminUser, "team-1")).toBe(
          true,
        );
      });

      it("denies team members from viewing team users", () => {
        expect(TeamAuthorization.Users.canView(teamMemberUser, "team-1")).toBe(
          false,
        );
      });

      it("denies non-members from viewing team users", () => {
        expect(TeamAuthorization.Users.canView(nonTeamUser, "team-1")).toBe(
          false,
        );
      });
    ...
    describe("canUpdate", () => {
      it("allows team admins to update team users", () => {
        expect(TeamAuthorization.Users.canUpdate(teamAdminUser, "team-1")).toBe(
          true,
        );
      });
      ...
      it("denies super admins from updating team users", () => {
        expect(
          TeamAuthorization.Users.canUpdate(superAdminUser, "team-1"),
        ).toBe(false);
      });

      it("denies team members from updating team users", () => {
        expect(
          TeamAuthorization.Users.canUpdate(teamMemberUser, "team-1"),
        ).toBe(false);
      });

      it("denies non-members from updating team users", () => {
        expect(TeamAuthorization.Users.canUpdate(nonTeamUser, "team-1")).toBe(
          false,
        );
      });
    ...
    describe("canInvite", () => {
      it("allows team admins to invite users to their team", () => {
        expect(TeamAuthorization.Users.canInvite(teamAdminUser, "team-1")).toBe(
          true,
        );
      });
      ...
      it("denies super admins from inviting users to teams", () => {
        expect(
          TeamAuthorization.Users.canInvite(superAdminUser, "team-1"),
        ).toBe(false);
      });

      it("denies team members from inviting users", () => {
        expect(
          TeamAuthorization.Users.canInvite(teamMemberUser, "team-1"),
        ).toBe(false);
      });

      it("denies non-members from inviting users", () => {
        expect(TeamAuthorization.Users.canInvite(nonTeamUser, "team-1")).toBe(
          false,
        );
      });
    ...
    describe("canRequestAccessToTeam", () => {
      it("allows super admins not in the team to request access", () => {
        expect(
          TeamAuthorization.Users.canRequestAccess(superAdminUser, "team-999"),
        ).toBe(true);
      });
      ...
      it("denies super admins already in the team from requesting access", () => {
        ...
        expect(
          TeamAuthorization.Users.canRequestAccess(superAdminInTeam, "team-1"),
        ).toBe(false);
      });

      it("denies team admins from requesting access", () => {
        expect(
          TeamAuthorization.Users.canRequestAccess(teamAdminUser, "team-1"),
        ).toBe(false);
      });

      it("denies team members from requesting access", () => {
        expect(
          TeamAuthorization.Users.canRequestAccess(teamMemberUser, "team-1"),
        ).toBe(false);
      });

      it("denies non members users from requesting access", () => {
        expect(
          TeamAuthorization.Users.canRequestAccess(nonTeamUser, "team-1"),
        ).toBe(false);
      });

      it("denies null users from requesting access", () => {
        expect(TeamAuthorization.Users.canRequestAccess(null, "team-1")).toBe(
          false,
        );
      });
    });
