  ...
  return (
    <>
      ...
      {(name.length > 0 && name.trim().length === 0) && (
        <Alert variant="destructive">
          <AlertCircleIcon />
          <AlertDescription>Team name must contain alpha numerical characters</AlertDescription>
        </Alert>
      )}
      {(name.length > 0 && name.trim().length > 0 && name.trim().length < 3) && (
        <Alert variant="destructive">
          <AlertCircleIcon />
          <AlertDescription>Team name must be longer than 3 characters</AlertDescription>
        </Alert>
      )}
