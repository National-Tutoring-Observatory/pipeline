...
describe("PromptAuthorization", () => {
  ...
  describe("canCreate", () => {
    ...
    it("denies super admins to create prompts in a team", () => {
      expect(PromptAuthorization.canCreate(superAdminUser, "team-1")).toBe(
        false,
      );
    });
  ...
  describe("canView", () => {
    it("allows team members to view prompts in their team", () => {
      expect(PromptAuthorization.canView(teamMemberUser, promptInTeam1)).toBe(
        true,
      );
    });

    it("allows team admins to view prompts in their team", () => {
      expect(PromptAuthorization.canView(teamAdminUser, promptInTeam1)).toBe(
        true,
      );
    });
    ...
    it("denies non-members from viewing prompts in another team", () => {
      expect(PromptAuthorization.canView(nonTeamUser, promptInTeam1)).toBe(
        false,
      );
    });
  ...
  describe("canUpdate", () => {
    it("allows team admins to update their own prompts", () => {
      expect(PromptAuthorization.canUpdate(teamAdminUser, promptInTeam1)).toBe(
        true,
      );
    });
    ...
    it("allows team members to update their own prompts", () => {
      expect(
        PromptAuthorization.canUpdate(
          teamMemberUser,
          promptCreatedByTeamMember,
        ),
      ).toBe(true);
    });
    ...
    it("allows team admins to update any prompt in their team", () => {
      expect(
        PromptAuthorization.canUpdate(teamAdminUser, promptCreatedByTeamMember),
      ).toBe(true);
    });

    it("denies team members from updating other members prompts", () => {
      expect(PromptAuthorization.canUpdate(teamMemberUser, promptInTeam1)).toBe(
        false,
      );
    });

    it("denies team members from updating prompts from other teams", () => {
      expect(PromptAuthorization.canUpdate(teamMemberUser, promptInTeam2)).toBe(
        false,
      );
    });

    it("denies super admins to update prompts in a team", () => {
      expect(PromptAuthorization.canUpdate(superAdminUser, promptInTeam1)).toBe(
        false,
      );
    });

    it("denies non-members from updating prompts", () => {
      expect(PromptAuthorization.canUpdate(nonTeamUser, promptInTeam1)).toBe(
        false,
      );
    });
  ...
  describe("canDelete", () => {
    it("allows team admins to delete their own prompts", () => {
      expect(PromptAuthorization.canDelete(teamAdminUser, promptInTeam1)).toBe(
        true,
      );
    });
    ...
    it("allows team admins to delete any prompt in their team", () => {
      expect(
        PromptAuthorization.canDelete(teamAdminUser, promptCreatedByTeamMember),
      ).toBe(true);
    });

    it("denies team members from deleting other users' prompts", () => {
      expect(PromptAuthorization.canDelete(teamMemberUser, promptInTeam1)).toBe(
        false,
      );
    });

    it("denies team members from deleting prompts from other teams", () => {
      expect(PromptAuthorization.canDelete(teamMemberUser, promptInTeam2)).toBe(
        false,
      );
    });

    it("denies super admins to delete prompts in a team", () => {
      expect(PromptAuthorization.canDelete(superAdminUser, promptInTeam1)).toBe(
        false,
      );
    });

    it("denies non-members from deleting prompts", () => {
      expect(PromptAuthorization.canDelete(nonTeamUser, promptInTeam1)).toBe(
        false,
      );
    });
  ...
  describe("multiple team memberships", () => {
    it("allows users to manage prompts in all their teams", () => {
      ...
      expect(
        PromptAuthorization.canDelete(multiTeamUser, multiTeamPrompt1),
      ).toBe(true);
      expect(
        PromptAuthorization.canDelete(multiTeamUser, multiTeamPrompt2),
      ).toBe(true);
    });
  });
});
