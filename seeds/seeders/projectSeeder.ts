...
async function processProjectFiles(documents: ReturnType<typeof getDocumentsAdapter>, projectId: string, teamId: string, fileNames: string[]) {
  ...
  const admins = await UserService.find({ role: 'SUPER_ADMIN' });
